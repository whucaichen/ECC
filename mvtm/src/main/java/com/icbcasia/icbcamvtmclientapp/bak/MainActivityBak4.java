...
public class MainActivityBak4 extends AppCompatActivity {
...
//        StatusBarUtil.setTransparent(MainActivity.this);
        StatusBarUtil.setTranslucent(MainActivityBak4.this, 50);
...
//                ActivityCompat.requestPermissions(this, new String[]{android.Manifest.permission.CAMERA}, 1);
    ...
    private void getAppDetailSettingIntent() {
        dialog = new AlertDialog.Builder(MainActivityBak4.this)
...
//        String ipStr = "218.4.33.199";
        int retCode = MobileCC.getInstance().setHostAddress(HOST_IP, HOST_PORT, true, MobileCC.SERVER_MS);
        if (retCode == 0) {
//            ipStr = "222.92.146.242";
            retCode = MobileCC.getInstance().setSIPServerAddress(SIP_IP, SIP_PORT);
            if (retCode == 0) {
    ...
    private void callECC() {
        MobileCC.getInstance().webChatCall(CODE_TEXT, "666", "666");
    }

    private void shouldExit() {
        ll_exit.setSelected(true);
        AlertDialog.Builder builder = new AlertDialog.Builder(MainActivityBak4.this);
...
//                        MainActivity.this.finish();
    ...
    private void showServiceEndDialog(String title) {
        if (isDismiss) return;
        isDismiss = true;
        LayoutInflater factory = LayoutInflater.from(MainActivityBak4.this);
        ...
        AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(MainActivityBak4.this);
...
//                    Toast.makeText(MainActivity.this, getString(R.string.login_fail) + broadMsg.getRequestCode().getErrorCode(), Toast.LENGTH_SHORT).show();
                    showServiceEndDialog(getString(R.string.login_fail));
                } else {
...
//                    Toast.makeText(MainActivity.this, getString(R.string.ecc_text_connect), Toast.LENGTH_SHORT).show();
                        if (MobileCC.getInstance().makeCall(CODE_VOICE, MobileCC.AUDIO_CALL, "666", "666") != 0) {
                            Log.e(TAG, "创建语音失败");
                            showServiceEndDialog(getString(R.string.call_fail_return));
                        } else {
                            Log.e(TAG, "创建语音成功");
                        }
                        isTexted = true;
...
//                        showServiceEndDialog(getString(R.string.ecc_update_error));
//                    }
//                    isCalled = true;
                    }
                } else {
                    Log.e(TAG, NotifyMessage.CALL_MSG_ON_CONNECTED + broadMsg.getRequestCode().getRetCode());
                }
                ...
                showServiceEndDialog(getString(R.string.ecc_service_end));
...
//                Toast.makeText(MainActivity.this, getString(R.string.queue_timeout), Toast.LENGTH_SHORT).show();
                showServiceEndDialog(getString(R.string.ecc_agent_busy));
...
//                    Toast.makeText(MainActivity.this, getString(R.string.net_loop_fail), Toast.LENGTH_SHORT).show();
                    showServiceEndDialog(getString(R.string.net_loop_fail));
                }
...
//                    Toast.makeText(this, R.string.switch_camera, Toast.LENGTH_SHORT).show();
    ...
    private boolean setAudioMode() {
        ...
        if (isHeadsetOn) {
            ...
            return MobileCC.getInstance().changeAudioRoute(MobileCC.AUDIO_ROUTE_RECEIVER);
        } else {
            Log.e(TAG, "setAudioMode " + isHeadsetOn);
        }
        return false;
    ...
    private void releaseResource() {
        if (isCalled) {
            ...
            isCalled = false;
            if (isTexted) handler.postDelayed(new Runnable() {
                @Override
                public void run() {
            ...
            MobileCC.getInstance().releaseWebChatCall();
            isTexted = false;
        } else MainActivityBak4.this.finish();
    }
}
